...
- static-website hosting for the sanitized JSON report;
- shared-key access disabled;
- OAuth as the default authorization mode;
- TLS 1.2 and HTTPS-only transport;
- browser CORS restricted to the configured origins;
...
- a reviewed retention, public-disclosure, and teardown decision for the sanitized output.
...
Set the Bicep `publicReportUrl` output as `live_report_url` in `docs/report-source.json`, or test it without a commit by passing it through the console's `report` query parameter.
...
11. The report storage account has shared-key access disabled and the expected managed-identity role assignment.
12. The publisher uploads a valid public envelope without raw evidence, credentials, private addresses, or unsupported root-cause claims.
13. GitHub Pages loads the live report, shows its provenance, and marks an expired envelope stale.
14. The console falls back to the committed fixture when the live endpoint is unavailable.
15. Logs, token access, certificate handling, public-output retention, backup, recovery, and teardown behavior are documented.
