...
WORKFLOW = ROOT / '.github/workflows/servicetracer-demo-api-extension-write-verify-only.yml'
ASSERTION = ROOT / 'workloads/servicetracer-demo-api/scripts/assert_extension_write_permission_verify_only.py'
AUTH = ROOT / '.project/authorizations/servicetracer-demo-api-extension-write-verify-only-20260725.json'


class ExtensionWriteVerifyOnlyTests(unittest.TestCase):
    def test_workflow_is_verify_only_and_protected(self) -> None:
    ...
    def test_authorization_is_bounded(self) -> None:
        ...
        self.assertEqual(
            record['assumptions']['rbac_bootstrap_success'],
            'assumed_by_human_authority_not_yet_durably_verified',
        )
