...
PR84_MERGE = "c96d9cbb765a023921fa819cf7d99c957e8ad608"
...
def main() -> None:
    ...
    require(anchors["pr84_merge_commit"] == PR84_MERGE, "PR #84 merge mismatch")
    require(anchors["pr84_ci_run_id"] == 30137351716, "PR #84 CI mismatch")
    require(
        anchors["pr84_source_vs_merge_file_content_difference_observed"] is False,
        "content-equivalence boundary regressed",
    )
    ...
    require(repo_sync["main_ahead_by_commits"] >= 83, "deployed-source comparison regressed")
    ...
    require(
        deployment["missing_action"] == "Microsoft.Compute/virtualMachines/extensions/write",
    ...
    require(
        operations["required_extension_write_effective"] in (False, "unverified"),
        "extension write effectiveness fabricated",
    )
    ...
    require(
        ...
        "cost observation mismatch",
    )

    frontend = api["frontend"]
    require(frontend["integration_merged_into_main"] is True, "frontend merge missing")
    require(
        frontend["github_pages_publication_verified_after_merge"] is False,
        "Pages publication fabricated",
    )
    require(
        frontend["live_browser_rendering_of_corrected_api_verified"] is False,
        "live browser verification fabricated",
    )

    require(reconciliation["baseline"]["main"] == PR84_MERGE, "historical reconciliation baseline changed")
    require(
        reconciliation["problem"]["classification"] == "canonical_shared_state_stale_after_pr84_merge",
        "classification mismatch",
    )
    require(reconciliation["problem"]["azure_contradiction"] is False, "false Azure contradiction")
    require(
        reconciliation["resolution"]["historical_planner_evidence_preserved"] is True,
        "planner history lost",
    )

    require(blocked["authorization"]["final_status"] == "consumed_blocked", "blocked evidence status mismatch")
    require(
        blocked["resolved_state"]["azure_resource_mutation_performed"] is False,
