...
PR82_MERGE = "5dfa3b76a9fb975002d9cd702a892a0f678c88c5"
...
def main() -> None:
    ...
    require(current["evidence_anchors"]["pr82_merge_commit"] == PR82_MERGE, "PR #82 merge mismatch")
    ...
    require(api["resolved_state"]["backend_transaction_success_verified"] is False, "backend boundary collapsed")
    ...
    require(historical["current_deployment_view"] is False, "historical planner record became current truth")

    reconciliation_baseline = reconciliation["baseline"]
    require(reconciliation_baseline["main"] == PR82_MERGE, "historical PR #82 reconciliation changed")
    require(reconciliation["problem"]["classification"] == "shared_state_view_stale", "problem classification changed")
    require(reconciliation["problem"]["azure_contradiction"] is False, "state drift became false Azure contradiction")
    ...
    require(inventory["combined_verification"]["actual_cost_observed"] is False, "historical inventory cost boundary changed")
