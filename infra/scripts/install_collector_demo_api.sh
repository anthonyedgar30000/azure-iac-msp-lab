...
CONFIG_ROOT='/etc/servicetracer'
ENV_FILE="${CONFIG_ROOT}/demo-api.env"
...
if ! id servicetracer >/dev/null 2>&1; then
  useradd --system --gid servicetracer --home-dir /var/lib/servicetracer --shell /usr/sbin/nologin servicetracer
fi
...
install -d -o root -g servicetracer -m 0750 "$APP_ROOT" "$CONFIG_ROOT"
install -o root -g servicetracer -m 0640 "$SOURCE_ROOT/demo_api/core.py" "$APP_ROOT/core.py"
install -o root -g servicetracer -m 0640 "$SOURCE_ROOT/demo_api/runtime.py" "$APP_ROOT/runtime.py"
install -o root -g servicetracer -m 0640 "$SOURCE_ROOT/demo_api/standalone_server.py" "$APP_ROOT/standalone_server.py"
...
EOF
chown root:servicetracer "$ENV_FILE"
chmod 0640 "$ENV_FILE"

cat > "/etc/systemd/system/${SERVICE_NAME}" <<EOF
[Unit]
Description=ServiceTracer collector-hosted demo API
...
cat > /etc/nginx/conf.d/servicetracer-demo-api-rate-limit.conf <<'EOF'
limit_req_zone $binary_remote_addr zone=servicetracer_demo_api:10m rate=6r/m;
EOF

rm -f /etc/nginx/sites-enabled/default
cat > /etc/nginx/sites-available/servicetracer-demo-api <<EOF
server {
    listen 80;
    listen [::]:80;
