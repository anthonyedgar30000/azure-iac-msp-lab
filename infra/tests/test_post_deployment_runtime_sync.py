...
class PostDeploymentRuntimeSyncTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.reality = json.loads(
            (ROOT / '.project/current-reality-v6.json').read_text(encoding='utf-8')
        )
        cls.index = json.loads(
            (ROOT / '.project/state-index-v15.json').read_text(encoding='utf-8')
        )
        cls.plan = json.loads(
            (
                ROOT
                / '.project/evidence/servicetracer-demo-api-plan-run-30660575435.json'
            ).read_text(encoding='utf-8')
        )
        cls.deploy = json.loads(
            (
                ROOT
        ...
        cls.recon = json.loads(
            (
                ROOT
                / '.project/reconciliations/servicetracer-plan-deploy-runtime-sync-20260731.json'
            ).read_text(encoding='utf-8')
        )
        cls.handoff = (
            ROOT / '.project/handoffs/current-state-v5.md'
        ).read_text(encoding='utf-8')

    def test_v6_state_is_retained_as_historical_compatibility(self) -> None:
        self.assertEqual(
        ...
        self.assertEqual(
            compatibility['.project/current-reality-v6.json'],
            'historical_post_deployment_pre_external_validation_reality',
        )
        self.assertEqual(
            compatibility['.project/state-index-v15.json'],
            'historical_post_deployment_pre_external_validation_index',
        )
        self.assertEqual(
            compatibility['.project/handoffs/current-state-v5.md'],
            'historical_post_deployment_pre_external_validation_handoff',
        )

    def test_plan_and_deployment_artifacts_remain_exact(self) -> None:
        self.assertEqual(self.plan['source']['workflow_run_id'], 30660575435)
        self.assertEqual(
            self.plan['source']['head_sha'],
            'ae08e7a72c14ec6deaddd3fa0f8c84b8342e0be3',
        )
        self.assertEqual(
            self.plan['artifact']['sha256'],
            '76911fb3b626786593df022bdfb654c01efde11cd0b3dc1809679c431f11d1f2',
        )
        self.assertEqual(
            self.plan['what_if']['status'],
            'accepted_independent_workload_create_plan',
        )
        self.assertEqual(self.deploy['source']['workflow_run_id'], 30661015789)
        self.assertEqual(
            self.deploy['artifact']['sha256'],
            '80b0819552623c13cfa244ec2d480f7606c2f224516a5d6c9882233fdbe2b478',
        )
        ...
        self.assertEqual(
            service['undeployed_repository_commits'],
            [
    ...
    def test_pre_external_validation_runtime_boundary_is_preserved(self) -> None:
        ...
        self.assertEqual(
            public['azure_host']['resource_group'],
    ...
    def test_historical_state_retains_no_operational_authority(self) -> None:
        self.assertTrue(
    ...
    def test_historical_handoff_documents_then_pending_gate(self) -> None:
