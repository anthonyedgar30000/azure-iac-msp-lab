...
SYNC = ROOT / ".project/reconciliations/post-pr246-canonical-sync-20260730.json"
...
BASE_MAIN = "8387c44aa4b82ae77cf32f53053187488d13d6ba"
PR246_SOURCE = "2a499d1c0cf71388705d03df893441e90a7f6cab"
...
class PostPr246CanonicalSyncTests(unittest.TestCase):
    @classmethod
    ...
    def test_selector_promotes_new_versioned_state(self) -> None:
    ...
    def test_repository_watermark_advances_through_pr246(self) -> None:
        repo = self.reality["repository_state"]
        self.assertEqual(repo["observed_main"], BASE_MAIN)
        self.assertEqual(repo["latest_merged_pull_request"], 246)
        self.assertEqual(repo["latest_merged_source_head"], PR246_SOURCE)
        self.assertEqual(repo["latest_exact_head_ci"]["run_id"], 30511634610)
        self.assertEqual(repo["latest_exact_head_ci"]["conclusion"], "success")
        self.assertEqual(repo["open_pull_requests_observed"], [])

    def test_run8_is_consumed_terminal_without_mutation(self) -> None:
        terminal = self.run8["result"]
        ...
        self.assertEqual(terminal["direct_account_scoped_matches"], 0)
        self.assertFalse(terminal["direct_account_role_verified"])
        boundary = self.run8["operation_boundary"]
        for key in (
        ...
        self.assertEqual(boundary["model_tokens_consumed"], 0)
    ...
    def test_promoted_run8_evidence_hash_matches_manifest(self) -> None:
        ...
        self.assertEqual(actual, "572489ee1380b239b4cd229ad1d99ef8aeb2e9831ee46366ed0b4664d4e417b7")
    ...
    def test_run8_request_is_terminalized(self) -> None:
        ...
        self.assertEqual(self.run8_request["terminal"]["workflow_run_id"], 30510660758)
        self.assertFalse(self.run8_request["terminal"]["rerun_authorized"])

    def test_servicetracer_planning_authority_remains_one_unconsumed_attempt(self) -> None:
        self.assertEqual(self.plan_request["status"], "authorized_pending_dispatch")
        self.assertTrue(self.plan_request["active"])
        self.assertEqual(self.plan_request["attempt_limit"], 1)
    ...
    def test_handoff_preserves_cost_and_execution_boundaries(self) -> None:
        self.assertIn(f"observed main: {BASE_MAIN}", self.handoff)
        self.assertIn("latest merged PR: #246", self.handoff)
        ...
        self.assertIn("actual Azure account cost freshly observed: false", self.handoff)
