...
class CollectorReplacementExecutionDesignTests(unittest.TestCase):
    ...
    def _validate(self, contract: dict) -> dict:
        namespace: dict[str, object] = {"__name__": "collector_replacement_execution_design_validator"}
    ...
    def test_candidate_is_not_an_active_github_workflow(self) -> None:
        ...
        self.assertEqual(self.contract["activation"]["candidate_workflow"], "infra/workflow-designs/collector-replacement-execution.yml")
    ...
    def test_workflow_design_fails_closed_before_azure_authentication(self) -> None:
        self.assertLess(self.workflow.index("Fail closed before Azure authentication or mutation"), self.workflow.index("Candidate replacement phase contract"))
    ...
    def test_candidate_contains_no_azure_mutation_commands(self) -> None:
        for forbidden in ("az vm delete", "az vm deallocate", "az snapshot create", "az disk create", "az disk update", "az resource update", "az deployment group create", "az role assignment create", "az network nic delete"):
    ...
    def test_contract_validates_as_fail_closed_and_not_promotion_ready(self) -> None:
        ...
        self.assertEqual(result["rollback_review_status"], "changes_addressed_re_review_pending")
    ...
    def test_contract_is_pinned_to_promoted_planner_evidence(self) -> None:
        ...
        self.assertEqual(result["evidence_anchor"]["planner_artifact_sha256"], "76f3b44e6e97e906dac6d62eeb212a6bc55265e77271a030b9c45b0cacf55637")
        self.assertEqual(result["target"]["exact_future_confirmation"], "REPLACE:rg-servicetracer-dev-westus2:vm-stcollector-mst-dev")

    def test_quiesce_and_deallocate_precedes_both_snapshots(self) -> None:
        result = self._validate(self.contract)
        phases = result["phase_order"]
        self.assertLess(phases.index("quiesce_and_deallocate_source"), phases.index("create_recovery_points"))
        ...
        self.assertEqual(set(boundary["snapshot_binding_fields"]), {"maintenance_correlation_id", "final_evidence_checkpoint_id", "final_evidence_checkpoint_sha256"})

    def test_quiesce_actions_are_exact_and_ordered(self) -> None:
        result = self._validate(self.contract)
        self.assertEqual(self.contract["consistency_boundary"]["ordered_actions"], EXPECTED_CONSISTENCY_ACTIONS)
        self.assertEqual(result["consistency_actions_exact_order"], EXPECTED_CONSISTENCY_ACTIONS)
    ...
    def test_validator_rejects_missing_collector_service_stop(self) -> None:
        modified = copy.deepcopy(self.contract)
        modified["consistency_boundary"]["ordered_actions"].remove("stop the collector service")
        with self.assertRaises(Exception):
            self._validate(modified)
    ...
    def test_validator_rejects_missing_checkpoint_binding(self) -> None:
        modified = copy.deepcopy(self.contract)
        modified["consistency_boundary"]["snapshot_binding_fields"].remove("final_evidence_checkpoint_sha256")
        with self.assertRaises(Exception):
            self._validate(modified)
    ...
    def test_isolated_exact_snapshot_boot_rehearsal_precedes_old_compute_removal(self) -> None:
        result = self._validate(self.contract)
        phases = result["phase_order"]
        self.assertLess(phases.index("isolated_snapshot_boot_rehearsal"), phases.index("remove_old_compute"))
    ...
    def test_rehearsal_teardown_is_required_before_replacement_compute(self) -> None:
        result = self._validate(self.contract)
        phases = result["phase_order"]
        self.assertLess(phases.index("isolated_snapshot_boot_rehearsal"), phases.index(result["rehearsal_teardown_before_phase"]))
        ...
        self.assertIn("temporary rehearsal VM and isolated NIC are removed before replacement compute", result["rehearsal_teardown_evidence_required"])
        self.assertEqual(result["approved_temporary_recovery_artifacts_may_remain"], ["verified OS-disk snapshot", "verified evidence-disk snapshot", "approved temporary recovery disk"])
    ...
    def test_rollback_separates_copy_from_attach(self) -> None:
        ...
        self.assertEqual(result["rollback_strategy"], "os_disk_snapshot_recreate_canonical_name")
        self.assertEqual(recreation["managed_disk_create_option"], "Copy")
        self.assertEqual(recreation["vm_os_disk_create_option"], "Attach")
        self.assertEqual(recreation["managed_disk_source"], "exact verified OS-disk snapshot")
        self.assertEqual(recreation["recreated_os_disk_name"], "disk-stcollector-os-mst-dev")

    def test_validator_rejects_from_image_for_snapshot_restore(self) -> None:
        modified = copy.deepcopy(self.contract)
        modified["rollback"]["recreation_contract"]["managed_disk_create_option"] = "FromImage"
        with self.assertRaises(Exception):
            self._validate(modified)

    def test_validator_rejects_missing_attach_semantics(self) -> None:
        modified = copy.deepcopy(self.contract)
        modified["rollback"]["recreation_contract"]["vm_os_disk_create_option"] = "FromImage"
        with self.assertRaises(Exception):
            self._validate(modified)

    def test_validator_rejects_recreation_metadata_drift(self) -> None:
        modified = copy.deepcopy(self.contract)
        modified["rollback"]["recreation_contract"]["metadata_required"].remove("disk SKU")
        with self.assertRaises(Exception):
            self._validate(modified)
    ...
    def test_validator_rejects_delete_semantics_for_preserved_attachments(self) -> None:
        for object_name, field in (("replacement_vm_attachment_contract", "nic_delete_option"), ("replacement_vm_attachment_contract", "evidence_disk_delete_option")):
            modified = copy.deepcopy(self.contract)
            modified[object_name][field] = "Delete"
            with self.assertRaises(Exception):
                self._validate(modified)
        for field in ("preserved_nic_delete_option", "preserved_evidence_disk_delete_option"):
            modified = copy.deepcopy(self.contract)
    ...
    def test_validator_rejects_missing_required_phase(self) -> None:
        modified = copy.deepcopy(self.contract)
        modified["phases"] = [phase for phase in modified["phases"] if phase["phase_id"] != "isolated_snapshot_boot_rehearsal"]
        with self.assertRaises(Exception):
            self._validate(modified)
    ...
    def test_cli_renders_deterministic_non_authorizing_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            result = subprocess.run(["python", str(VALIDATOR_PATH), "--contract", str(CONTRACT_PATH), "--output", str(output)], cwd=ROOT, text=True, capture_output=True, check=False)
            ...
            self.assertTrue(rendered["rehearsal_temporary_compute_removed_before_replacement"])
