...
from infra.scripts.authority_claim_v2 import (
    ClaimContext,
...
class AzureMcpLiveActivationTests(unittest.TestCase):
    def test_runtime_is_digest_pinned_scale_to_zero_and_read_only(self) -> None:
        ...
        for namespace in ("'compute'", "'monitor'"):
    ...
    def test_least_privilege_scope_is_target_resource_group(self) -> None:
        ...
        self.assertEqual(scope["scope"]["role_assignments"][0]["scope"].split("/")[-1], "rg-servicetracer-dev-westus2")

    def test_entra_application_and_service_principal_are_separate_from_azure_what_if(self) -> None:
        main = MAIN.read_text(encoding="utf-8")
        entra = ENTRA.read_text(encoding="utf-8")
        deploy = DEPLOY.read_text(encoding="utf-8")
        self.assertNotIn("Microsoft.Graph", main)
        self.assertIn("Microsoft.Graph/applications@v1.0", entra)
        self.assertIn("Microsoft.Graph/servicePrincipals@v1.0", entra)
        self.assertLess(deploy.index("az deployment sub what-if"), deploy.index("Deploy Entra application and service principal"))
        self.assertIn("unsupported for extensible Microsoft Graph resources", (ROOT / ".project/contracts/azure-mcp-live-activation-v1.json").read_text(encoding="utf-8"))
    ...
    def test_deployment_claims_before_oidc_and_rejects_reruns(self) -> None:
        ...
        self.assertIn("observed_scope_hash", deploy)
